...
// Called by spindle_sync() after sync and parking motion/spindle stop override during restore.
    // Sets servo running state servo PWM.
#ifdef VARIABLE_SPINDLE
  void servo_set_state(uint8_t state, float rpm); 
    // Sets servo PWM quickly for stepper ISR. Also called by serv_set_state().
  // NOTE: 328p PWM register is 8-bit.
  void servo_set_pos(uint8_t pwm_value);
#endif
#ifdef VARIABLE_SPINDLE
